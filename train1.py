...
class DPVGAE(nn.Module):
    ...
    def forward(self, data):
        ...
        edge_view1 = edge_index_pur
        edge_view2 = edge_index_pur[:, perm[:keep]]
        h1 = self.gin(z_k, edge_view1)
...
def train_linkpred(model, splits, args, device="cpu"):
    ...
    for epoch in tqdm(range(1, 1 + args.epochs)):
        model.train()
        optimizer.zero_grad()
        loss, loss_dict = model(train_data)
        loss.backward()
        if args.grad_norm > 0:
...
parser.add_argument("--dataset", nargs="?", default="Cora", help="Dataset name")
...
parser.add_argument("--save_path", default="DPVGAE-LinkPred.pt")
...
model = DPVGAE(data.num_features, data.num_nodes, args).to(device)
...
for run in range(1, args.runs+1):
    ...
    print(f'Run {run} - AUC: {test_auc:.4f}, AP: {test_ap:.4f}')

print(f'\nLink Prediction Results ({args.runs} runs):')
print(f'AUC: {np.mean(auc_results):.4f} ± {np.std(auc_results):.4f}')
print(f'AP:  {np.mean(ap_results):.4f} ± {np.std(ap_results):.4f}')